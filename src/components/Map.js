...
export default function Map() {
  ...
  const drop2 = expand
    ? { height: "0.1" }
    : { margin: "0", display: "flex", padding: "0" };
  const drop3 = expand
    ? { height: "0.1", opacity: "0", transform: "translateY(-30%)" }
    : { opacity: "1", transform: "translateY(-70%)" };
  const drop4 = expand
    ? { height: "10px", paddingTop: "40px" }
    ...
    <div id="main-app">
      ...
      <div id={styleStart} onClick={click}>
        ...
        <div id="years" style={drop4}>
          ...
          <button style={butstyle} id="done" onClick={reverseClick}>
            ↓
          </button>
        </div>
      </div>
      <MapContainer
        ...
        minZoom={10}
        zoomControl={false}
        style={{ background: "#010c17", transition: " all 2s" }}
      >
        <TileLayer
        ...
        {Data.map((hood) => {
          ...
          const color = getColor(rentIncreasePercentage);
